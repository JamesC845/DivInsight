...
To get data for an exact location a pair of longitude/latitude coordinates can be used with `subset_by_coordinate_ref()`. 

The `coordinate_reference` argument will mark the centre of the location and the `distance_threshold` will mark the radius of location.

Below this function is used as an argument value in the `clusterise_sites()` function to get clusterised data for two locations.
